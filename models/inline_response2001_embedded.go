		...
		if m.ActivityReports[i] != nil {
			if err := m.ActivityReports[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					...
					return ce.ValidateName("activity_reports" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
...
// ContextValidate validate this inline response 200 1 embedded based on the context it is used
func (m *InlineResponse2001Embedded) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	var res []error
...
func (m *InlineResponse2001Embedded) contextValidateActivityReports(ctx context.Context, formats strfmt.Registry) error {

	for i := 0; i < len(m.ActivityReports); i++ {

		if m.ActivityReports[i] != nil {
			if err := m.ActivityReports[i].ContextValidate(ctx, formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("activity_reports" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("activity_reports" + "." + strconv.Itoa(i))
