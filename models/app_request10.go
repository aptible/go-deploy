// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// AppRequest10 app request 10
//
// swagger:model app_request_10
type AppRequest10 struct {

	// handle
	Handle string `json:"handle,omitempty"`
}

// Validate validates this app request 10
func (m *AppRequest10) Validate(formats strfmt.Registry) error {
	return nil
}

// ContextValidate validates this app request 10 based on context it is used
func (m *AppRequest10) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *AppRequest10) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *AppRequest10) UnmarshalBinary(b []byte) error {
	var res AppRequest10
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
